              ...
              <div class="col-sm-12">
                <div class="home-tab">
                  ...
                  <div class="tab-content tab-content-basic">
                    <?php $query = mysqli_fetch_assoc($link->query("SELECT COUNT(CASE WHEN report_status='Approved' THEN 1 END) as 'approved', 
                    COUNT(CASE WHEN report_status='Requesting' THEN 1 END) as 'Requesting',
                    COUNT(CASE WHEN report_status='Need Edit' THEN 1 END) as 'NeedEdit' 
                    FROM report LEFT JOIN bfp_users ON bfp_users.uid = report.uid 
                    ...
                    <div class="tab-pane fade show active" id="Overview" role="tabpanel" aria-labelledby="Overview">
                            ...
                            <div class="card-header fw-bold d-flex justify-content-between">
                              ...
                              <div class="dropdown">
                                ...
                                <ul class="dropdown-menu">
                                  <li><a class="dropdown-item" href="municipal.php">View Report</a></li>
                                </ul>
                              </div>
                            </div>
                              ...
                              <script>
                                ...
                                var yArray = [<?php
                                              for($a=$minus5year; $a<=$now;$a++){
                                                ...
                                                WHERE bfp_form1.form1_date_added BETWEEN '$a-1-1 00-00-01' AND '$a-12-$lastday 23-59-59'
                                                AND bfp_users.municipal = '".$sh_user['municipal']."' AND bfp_form1.form1_status = '1'"));
                                ...
                                var zArray = [<?php
                                              for($a=$minus5year; $a<=$now;$a++){
                                                ...
                                                AND bfp_users.municipal = '".$sh_user['municipal']."' AND bfp_form2.form2_status = '1';"));
                                ...
                                var cArray = [<?php
                                              for($a=$minus5year; $a<=$now;$a++){
                                                ...
                                                AND bfp_users.municipal = '".$sh_user['municipal']."' AND bfp_form3.form3_status = '1';"));
                      ...
                      <div class="row">
                        <div class="col-lg-12 mt-3">
                          <div class="card chart-container p-2" style="position: relative;height:auto; width:78vw">
                            <canvas id="municipalchart"></canvas>
                            <?php $municipalQ = $link->query("SELECT * FROM bfp_users
                            WHERE municipal = '".$sh_user['municipal']."' AND uid != '$uid' AND status = 'true' ORDER BY location ASC")?>
                            <?php $f1municipal_Q = $link->query("SELECT * FROM bfp_users
                            WHERE municipal = '".$sh_user['municipal']."' AND uid != '$uid' AND status = 'true' ORDER BY location ASC")?>
                            <?php $f2municipal_Q = $link->query("SELECT * FROM bfp_users
                            WHERE municipal = '".$sh_user['municipal']."' AND uid != '$uid' AND status = 'true' ORDER BY location ASC")?>
                            <?php $f3municipal_Q = $link->query("SELECT * FROM bfp_users
                            WHERE municipal = '".$sh_user['municipal']."' AND uid != '$uid' AND status = 'true' ORDER BY location ASC")?>
                            <?php $f4municipal_Q = $link->query("SELECT * FROM bfp_users
                            WHERE municipal = '".$sh_user['municipal']."' AND uid != '$uid' AND status = 'true' ORDER BY location ASC")?>
                            <script>
                            var xValues = [
                              <?php while($muncipalSh = $municipalQ->fetch_assoc()){?>
                                "<?php echo $muncipalSh['location']?>",
                              <?php }?>
                            ];
                            var f1Values = [
                              <?php while($f1municipal = $f1municipal_Q->fetch_assoc()){
                                $f1count = mysqli_fetch_assoc($link->query("SELECT COUNT(bfp_users.uid) FROM bfp_form1
                                LEFT JOIN bfp_users ON bfp_users.uid = bfp_form1.uid
                                WHERE bfp_users.municipal = '".$sh_user['municipal']."' AND bfp_users.location = '".$f1municipal['location']."'
                                AND bfp_form1.form1_status = '1' 
                                ORDER BY bfp_users.location ASC"));
                                echo $f1count['COUNT(bfp_users.uid)'].','; 
                              }?>
                            ];
                            var f2Values = [
                              <?php while($f2municipal = $f2municipal_Q->fetch_assoc()){
                                $f2count = mysqli_fetch_assoc($link->query("SELECT COUNT(bfp_users.uid) FROM bfp_form2
                                LEFT JOIN bfp_users ON bfp_users.uid = bfp_form2.uid
                                WHERE bfp_users.municipal = '".$sh_user['municipal']."' AND bfp_users.location = '".$f2municipal['location']."'
                                AND bfp_form2.form2_status = '1' 
                                ORDER BY bfp_users.location ASC"));
                                echo $f2count['COUNT(bfp_users.uid)'].','; 
                              }?>
                            ];
                            var f3Values = [
                              <?php while($f3municipal = $f3municipal_Q->fetch_assoc()){
                                $f3count = mysqli_fetch_assoc($link->query("SELECT COUNT(bfp_users.uid) FROM bfp_form3
                                LEFT JOIN bfp_users ON bfp_users.uid = bfp_form3.uid
                                WHERE bfp_users.municipal = '".$sh_user['municipal']."' AND bfp_users.location = '".$f3municipal['location']."'
                                AND bfp_form3.form3_status = '1' 
                                ORDER BY bfp_users.location ASC"));
                                echo $f3count['COUNT(bfp_users.uid)'].','; 
                              }?>
                            ];
                            var f4Values = [
                              <?php while($f4municipal = $f4municipal_Q->fetch_assoc()){
                                $f4count = mysqli_fetch_assoc($link->query("SELECT COUNT(bfp_users.uid) FROM bfp_form4
                                LEFT JOIN bfp_users ON bfp_users.uid = bfp_form4.uid
                                WHERE bfp_users.municipal = '".$sh_user['municipal']."' AND bfp_users.location = '".$f4municipal['location']."'
                                AND bfp_form4.form4_status = '1' 
                                ORDER BY bfp_users.location ASC"));
                                echo $f4count['COUNT(bfp_users.uid)'].','; 
                              }?>
                            ];
                            ...
                            new Chart("municipalchart", {
                              ...
                              options: {
                                ...
                                title: {
                                  display: true,
                                  text: "Total Application Per Municipal",
                                  fontSize: 18
                                },scales: {
                                  yAxes: [{ticks: {min: 0}}]
                                }
          ...
          <div class="modal fade" id="excelmodal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                ...
                <form id="excelform" action="../province-sheet.php" method="GET" class="modal-body">
                  <div class="row">
                  <input type="hidden" value="<?php echo $sh_user['municipal']?>" name="office" id="office" class="form-control" required>
                    <div class="col-lg-6">
                      Start
              ...
              <div class="modal-content">
                ...
                <form id="pdfform" action="../pdfreport.php" method="GET" target="_blank" class="modal-body">
                  <div class="row">
                    ...
                    <input type="hidden" name="province" value="<?php echo $sh_user['municipal']?>" id="">
                    <div class="col-lg-6">
                      Start
                      <input type="date" value="<?php echo date('Y-m-01')?>" name="start" id="start" class="form-control" required>
                    </div>
